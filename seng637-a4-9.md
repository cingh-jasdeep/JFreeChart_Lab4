...
This is also the case for SkyScanner, FlightHub, skiplagged etc. It was visble that these dynamic search content websites have classic problems while testting in Selenium IDE. So we chose an alternative website to test - Wikipedia.

### Wikipedia

The Wikipedia test scenarios focus on validating core user interactions using stable and reliable element locators such as IDs and structured CSS selectors. Emphasis was placed on verifying functional correctness through page headings and content validation rather than relying on fragile UI attributes. Additionally, wait conditions were incorporated to handle asynchronous loading and reduce test flakiness. While the tests primarily cover positive user flows, they provide a strong foundation for validating navigation consistency and content accuracy across the application.
...
The `assertElementPresent` command was used to verify that the main article content is displayed after performing a search. Additionally, the `assertText` command was used to ensure that the page heading matches the expected article title ("COVID-19"). Stable selectors such as element IDs were prioritized to reduce flakiness caused by dynamic DOM updates. These assertions confirm that the search operation successfully navigates to the correct page.

### Wikipedia: Navigation Tabs

The `assertElementPresent` command was used after each navigation action (View source and View history) to confirm that the page content has loaded. The `assertText` command was used to validate that the heading reflects the correct page (e.g., "View source for Main Page" and "Main Page: Revision history"). Using headings as validation points ensures that navigation results are functionally correct rather than just visually loaded.

### Wikipedia: Section Navigation

The `assertElementPresent` command was used to verify that the target section content is visible after clicking on "External links" in the table of contents. The `assertText` command was also used to confirm that the section heading matches "External links". This ensures that internal anchor-based navigation works correctly within long documents.

### Wikipedia: Current Events Page

The `assertElementPresent` command was used to confirm that the main content of the Current events page is displayed after navigation. The `assertText` command was used to validate that the page heading is "Current events". Additionally, a checkpoint using `assertElementPresent` was used to verify the presence of the calendar section, ensuring that key page components are loaded correctly. Wait commands were implicitly used as checkpoints to handle asynchronous content loading and improve test reliability.
...
The search functionality was tested using the input "covid-19" to verify that the system correctly retrieves and displays the relevant article. A widely recognized and unambiguous term was chosen to ensure deterministic results and avoid inconsistencies due to similar or ambiguous search queries. This helps validate both search accuracy and correct page navigation.

### Wikipedia: Navigation (View Source & History)

The navigation functionality was tested by interacting with "View source" and "View history" options on the Main Page. These actions represent different navigation paths and page states, ensuring that the system consistently loads the correct pages with appropriate headings. Testing multiple navigation options improves confidence in link reliability and page transitions.

### Wikipedia: Section Navigation (External Links)

The section navigation was tested by selecting the "External links" option from the table of contents. This verifies that internal anchor links correctly navigate to the intended section. Such tests ensure that long-page navigation mechanisms function correctly and provide accurate positioning within the document.

### Wikipedia: Current Events Page

The current events functionality was tested by navigating through the side menu to the "Current events" page. This validates that menu-driven navigation works correctly and that key dynamic components, such as headings and calendar sections, are rendered properly. This test ensures correct behavior across different content types and layouts.
