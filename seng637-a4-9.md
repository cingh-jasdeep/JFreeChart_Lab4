...
- Tries to search for a paragraph with the selected language’s local name, stores the result in a variable and checks if the variable contains the selected language’s local name
...
The `assert` command was used to assert that the languages for a course contain the language selected earlier to filter the courses.
