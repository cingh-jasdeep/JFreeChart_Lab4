...
### Air Canada (Alteranate Skiplagged) : Flight Search
...
This is also the case for SkyScanner and FlightHub. But we found an alternative Skiplagged which averted this problem.

Experiment Setup 1 (Skiplagged)

- Open the website
- Selected the trip type
- Selecty Round trip
- Clicked on the departure field
- Type "YYC"
- Selected an option from the suggestions
- Clicked on the arrival field
- Type "YVR"
- Selected an option from the suggestions
- Picked a departure date
- Picked a return date
...
| ---------------------- | -------------------------------------------------------------------- | ----------- | -------------------------------- |
| assert element present | cid=trip-list-wrapper                                                |             | search result found              |
| assert value           | css=input[data-testid="FlightSearch.TripRow.testFields.fromAirport"] | YYC         | departrure set correctly         |
| assert value           | css=input[data-testid="FlightSearch.TripRow.testFields.toAirport"]   | YVR         | test - destination set correctly |
...
| assert text            | css=#destination-error | Invalid City | error mesasage shown correctly |
...
- Type "YVR"
- Selected an option from the suggestions
- Picked a departure date - 5 Apr
- Picked a return that's before departure date - 20 Mar

Test Cases

| Command                | Target                                                                       | Value | Description (Objective) |
| ---------------------- | ---------------------------------------------------------------------------- | ----- | ----------------------- |
| assert element present | xpath=//td[contains(@class,'ui-datepicker-unselectable')]//span[text()='20'] |       | past date found         |
| click                  | xpath=//span[text()='20']                                                    |       | select past date        |
| assert not text        | css=input.end-date                                                           | 20    | past date not selected  |
